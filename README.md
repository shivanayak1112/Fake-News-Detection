...
│                    FRONTEND (React @ 5173)                      │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ Dashboard (Model Metrics) | Analyze (Text/URL Input)   │   │
│  └─────────────────────────────────────────────────────┬───┘   │
└──────────────────────────────────────────────────────┼──────────┘
                                                       │ HTTP/CORS
                ┌──────────────────────────────────────┘
...
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  /api/analyze  │  /api/health  │  /api/metrics         │    │
│  └────────┬───────────────────────────────────────────────┘    │
└───────────┼──────────────────────────────────────────────────────┘
            ↓
┌──────────────────────────────────────────────────────────────────┐
...
│  ┌──────────┐  ┌────────────┐  ┌──────────────┐  ┌─────────┐   │
│  │ML Service│→ │   Claim    │→ │   Evidence   │→ │Decision │   │
│  │(TF-IDF + │  │ Extractor  │  │  Retrieval   │  │ Engine  │   │
│  │  LR)     │  │            │  │              │  │         │   │
│  └──────────┘  └────────────┘  └──────────────┘  └────┬────┘   │
└────────────────────────────────────────────────────┼───────────┘
                                                     ↓
...
│ • Stance classification (per evidence) │
│   - SUPPORTED                          │
│   - CONTRADICTED                       │
│   - INSUFFICIENT_INFO                  │
...
│ • Weight ML signal: 35%                │
│ • Weight evidence: 65%                 │
│   - Supported claims boost confidence  │
│   - Contradicted claims lower it       │
│ • Synthesize final verdict             │
│ OUTPUT: {decision, confidence, factors}│
└────────────────┬──────────────────────┘
                 ↓
┌───────────────────────────────────────┐
│ 7. EXPLANATION GENERATION             │
│ • Summarize claims analysis            │
│ • List supporting evidence sources     │
│ • Highlight contradicting evidence     │
│ • Present decision reasoning           │
│ OUTPUT: human-readable explanation     │
└────────────────┬──────────────────────┘
                 ↓
┌───────────────────────────────────────┐
│ RETURN TO FRONTEND                    │
│ • Display verdict (REAL/FAKE)          │
│ • Show confidence meter                │
│ • List claims & evidence               │
│ • Display explanation                  │
└───────────────────────────────────────┘
```
...
|-----------|-----------------|------------|
| **React Frontend** | User interface, results visualization, navigation | React 19.2, Vite, Axios |
| **FastAPI Backend** | Request routing, orchestration, response formatting | FastAPI 0.110+, Uvicorn |
| **ML Service** | Text classification, confidence scoring | scikit-learn, joblib |
| **Claim Extractor** | Factual claim identification from text | spaCy-inspired heuristics |
| **Evidence Retriever** | Multi-source information gathering | Google Fact Check, Wikipedia, Web APIs |
| **Decision Engine** | Hybrid verdict synthesis (ML + evidence) | Custom scoring algorithm |
| **Explanation Service** | Human-readable result narratives | Template-based generation |
